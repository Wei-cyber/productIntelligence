...
Reads the raw SQL templates from warehouse/merge_queries.sql rather than
inlining them in Python, so the merge logic stays reviewable/testable on
its own and isn't buried in app code.
"""
...
def _load_merge_statements() -> dict:
    ...
    comment headers. We only need statements 1 and 2 (the commented-out
    analytical query at the bottom is for ad-hoc use, not load.py).
    """
...
def fetch_unprocessed_bronze(engine, since: datetime = None) -> list[dict]:
    ...
    everything newer than `since` (default: last 1 day) — fine for a daily
    batch job. A watermark table would be the next step if volume grows.
    """
